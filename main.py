...
def decode(model, my_data, use_gpu):
    ...
    model.eval()
    with codecs.open("sentence.json", 'r', 'utf-8') as f1:
    ...
    encoder_output,vanilla_attention,universal_unify = model.encoder(sentence_tensor, sentence_mask, use_gpu,return_attn=True)
    # vanilla_attention = np.squeeze(vanilla_attention,axis=1)
    # np.save('vanilla_attention', vanilla_attention)
    #np.save('transition_matrix',model.decoder.transition_matrix.cpu().detach().numpy())
    np.save("universal_updates",universal_unify)
    path = model.decoder(encoder_output, sentence_mask, use_gpu)
    # 然后，我们在控制台输出结果
    # 因为我们只有一个句子，所以我们取第一个
    path = path[0]
    for i, tag_idx in enumerate(path):
        print(sentence_list[i], my_data.tag_vocab.itos[tag_idx])
...
def train_model(model, config, args, my_data, use_gpu):
    ...
    print_interval = 100 # 每十个batch输出一次
    # 准备优化器
...
if __name__ == '__main__':
    ...
    opt.add_argument('--train', help='the path of train file', default='data/MSRA2/msra_train_bio')
    opt.add_argument('--dev', help='the path of dev file', default='data/MSRA2/msra_dev_bio')
    opt.add_argument('--test', help='the path of test file', default='data/MSRA2/msra_test_bio')
    args = opt.parse_args()
    # gpu 是否可以使用
    ...
    if status == 'train':
        ...
        print('test_result:  time:{:.2f}, accuracy:{:.4f}, '
